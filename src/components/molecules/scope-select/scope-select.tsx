    ...
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      */}
      <Combobox
        value={selected}
        by="id"
        onChange={(next: ScopeOption | null) => onChange(!next || next.id === ALL ? null : next.id)}
        onClose={() => setQuery('')}
        disabled={disabled}
      ...
      >
        {({ open: isOpen }) => (
          <div ref={triggerRef} className="relative">
            <ComboboxButton className="border-line focus-visible:ring-accent focus-visible:ring-offset-page flex w-full items-center justify-between gap-2 rounded-md border bg-tint/[0.06] px-3 py-2 text-left transition-colors duration-150 hover:bg-tint/[0.1] focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-40">
              <Typography
